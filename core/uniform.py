...
class Uniform(object):
    ...
    def locateVariable(self, programRef, variableName):
        if self.dataType == "Light":
            ...
            self.variableRef = {}
            self.variableRef["lightDirection"] = glGetUniformLocation(programRef, 
                                                                variableName + ".lightDirection")
            self.variableRef["projectionMatrix"] = glGetUniformLocation(programRef, 
                                                                variableName + ".projectionMatrix")
            self.variableRef["viewMatrix"] = glGetUniformLocation(programRef, variableName + 
                                                                ".viewMatrix")
            self.variableRef["depthTexture"] = glGetUniformLocation(programRef, variableName + 
                                                                ".depthTexture")
            self.variableRef["strength"] = glGetUniformLocation(programRef, variableName +
                                                                ".strength")
            self.variableRef["bias"] = glGetUniformLocation(programRef, variableName + 
                                                                ".bias")
        else:
    ...
    def uploadData(self):
        ...
        if self.dataType == "int":
            ...
            glUniform3f(self.variableRef["color"], self.data.color[0], self.data.color[1],
                        self.data.color[2])
            ...
            glUniformMatrix4fv(self.variableRef["projectionMatrix"], 1, GL_TRUE,
                                self.data.camera.projectionMatrix)
            glUniformMatrix4fv(self.variableRef["viewMatrix"], 1, GL_TRUE, 
                                self.data.camera.viewMatrix)
