...
class Base(object):
    ...
    def __init__(self, screenSize=[512,512]):
        ...
        #user a core OpenGL profile for cross-platform compatibility
    ...
    def run(self):
        ...
        #mainloop
        while self.running:
            if self.input.nextState is not None:
                self.currentState = self.stateDict[self.input.nextState]
            ## process input ##
            self.input.update()
            if self.input.quit:
                self.running = False
            ...
            self.input.updatePreviousMousePos()
