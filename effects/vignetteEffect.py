    ...
    def __init__(self, dimStart=0.4, dimEnd=1.0, dimColor=[0,0,0]):
        ...
        fragmentShaderCode = """
            ...
            void main()
            {
                ...
                //calculate position in clip sapce from UV coordinates
                vec2 position = 2 * UV - vec2(1,1);
                //calculate distance (d) from center, which affects brightness
                float d = length(position);
                //calculate brightness (b) factor: when d=dimStart, b=1, when d=dimEnd, b=0
                float b = (d - dimEnd) / (dimStart - dimEnd);
                //prevent oversaturation
                b = clamp(b, 0, 1);
                //mis the texture color and dim color
                fragColor = vec4(b * color.rbg + (1-b) * dimColor, 1);
            }
        """
